    ...
    def __init__(self, pretrained='imagenet'):
        ...
        if pretrained is not None:
            self.base_model.load_state_dict(
                torch.load(
                    "c:/work/se_resnext50_32x4d-a260b3a4.pth"
                )
            )
...
def train(fold):
    ...
    training_data_path = "c:/work/train"
    df = pd.read_csv("c:/work/train_folds.csv")
    device = "cuda"
    epochs = 10
    train_bs = 32
    valid_bs = 16
...
def predict(fold):
    test_data_path = "c:/work/test/"
    df = pd.read_csv("c:/work/test.csv")
    device = "cuda"
